...
# Build a unique flavors dictionary keyed by "title". For duplicate titles, combine the review texts.
unique_flavors = {}
for doc in docs:
...
#   "reviews" from the aggregated "text" field
composite_models = build_composite_svd_models(flavor_list, n_components=50)

# Define composite weights for each field (these should sum to 1.0; adjust as needed)
weights = {
    "description": 0.4,
    "subhead": 0.3,
    "ingredients": 0.2,
...
def json_search(query: str) -> str:
    ...
    # Build list of scored flavors (only include those with a score > 0)
    scored_flavors = []
    ...
    # Prepare and return the top 10 recommendations
    out = []
